...
import { toastKey, toastSessionStorage, type ToastInput } from '#app/utils/toast.server.ts'
...
expect.extend({
	...
	async toSendToast(response: Response, toast: ToastInput) {
		...
		return {
			pass,
			message: () =>
				`toast in the response ${this.isNot ? 'does not match' : 'matches'} the expected toast${diff}`,
		}
	},
})
